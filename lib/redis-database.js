...
class RedisDatabase {
  ...
  async deleteStudent(studentId) {
    try {
      ...
      // Delete all history entries (scan for date-specific keys with timeout protection)
      const historyKeys = [];
      let cursor = 0;
      let scanCount = 0;
      ...
      do {
        const result = await this.redis.scan(cursor, { match: `history:${studentId}:*`, count: 100 });
      ...
      if (historyKeys.length > 0) {
        ...
        const batchSize = 50;
        for (let i = 0; i < historyKeys.length; i += batchSize) {
          const batch = historyKeys.slice(i, i + batchSize);
          await this.redis.del(...batch);
        }
      }

      // Delete all goal entries (scan for date-specific keys with timeout protection)
      const goalKeys = [];
      cursor = 0;
      scanCount = 0;
      
      do {
        const result = await this.redis.scan(cursor, { match: `goals:${studentId}:*`, count: 100 });
        cursor = parseInt(result[0]);
        goalKeys.push(...result[1]);
        scanCount++;
      } while (cursor !== 0 && scanCount < maxScans);
      ...
      if (goalKeys.length > 0) {
        // Delete in batches to avoid large operations
        const batchSize = 50;
        for (let i = 0; i < goalKeys.length; i += batchSize) {
          const batch = goalKeys.slice(i, i + batchSize);
          await this.redis.del(...batch);
        }
      }
