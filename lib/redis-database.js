...
class RedisDatabase {
  ...
  async deleteStudent(studentId) {
    try {
      ...
      // Delete all history entries (scan for date-specific keys)
      const historyKeys = [];
      let cursor = 0;
      do {
        const result = await this.redis.scan(cursor, { match: `history:${studentId}:*` });
        cursor = result[0];
      ...
      // Delete all goal entries (scan for date-specific keys)
      const goalKeys = [];
      cursor = 0;
      do {
        const result = await this.redis.scan(cursor, { match: `goals:${studentId}:*` });
        cursor = result[0];
        goalKeys.push(...result[1]);
      } while (cursor !== 0);
      
      if (goalKeys.length > 0) {
        await this.redis.del(...goalKeys);
      }
