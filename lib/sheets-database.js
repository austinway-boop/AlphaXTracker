...
class SheetsDatabase {
  ...
  async getAllStudents() {
    ...
    try {
      ...
      headers.forEach((header, index) => {
        ...
        if (lowerHeader === 'id') mapping.id = index;
        else if (lowerHeader.includes('firstname')) mapping.firstName = index;
        else if (lowerHeader.includes('lastname')) mapping.lastName = index;
        else if (lowerHeader === 'email') mapping.email = index;
        else if (lowerHeader === 'house') mapping.house = index;
        else if (lowerHeader === 'honors') mapping.honors = index;
        else if (lowerHeader.includes('dailygoal')) mapping.dailyGoal = index;
        else if (lowerHeader.includes('sessiongoal')) mapping.sessionGoal = index;
        else if (lowerHeader.includes('projectoneliner')) mapping.projectOneliner = index;
        else if (lowerHeader.includes('instagramgoal')) mapping.instagramGoal = index;
        else if (lowerHeader.includes('tiktokgoal')) mapping.tiktokGoal = index;
        else if (lowerHeader.includes('youtubegoal')) mapping.youtubeGoal = index;
      });

      // Use default mapping if headers aren't found
      ...
      const students = rows.slice(1).map(row => {
        ...
        return {
          id: parseInt(row[mapping.id]) || 0,
          firstName: firstName,
          lastName: lastName,
          email: row[mapping.email] || '',
          house: row[mapping.house] || '',
          honors: row[mapping.honors] === 'true' || row[mapping.honors] === 'TRUE' || row[mapping.honors] === true,
          dailyGoal: row[mapping.dailyGoal] || '',
          sessionGoal: row[mapping.sessionGoal] || '',
          projectOneliner: row[mapping.projectOneliner] || '',
          instagramGoal: parseInt(row[mapping.instagramGoal]) || 2,
          tiktokGoal: parseInt(row[mapping.tiktokGoal]) || 2,
          youtubeGoal: parseInt(row[mapping.youtubeGoal]) || 2,
  ...
  async updateStudent(studentId, updates) {
    try {
      ...
      // Correct column order: ID, First Name, Last Name, Email, House, Honors, Daily Goal, Session Goal, Project Oneliner, Instagram Goal, TikTok Goal, YouTube Goal
      const row = [
        updatedStudent.id,
        updatedStudent.firstName || '',
        updatedStudent.lastName || '',
        updatedStudent.email || '',
        updatedStudent.house || '',
        String(updatedStudent.honors || false),
        updatedStudent.dailyGoal || '',
        updatedStudent.sessionGoal || '',
        updatedStudent.projectOneliner || '',
        updatedStudent.instagramGoal || 2,
        updatedStudent.tiktokGoal || 2,
        updatedStudent.youtubeGoal || 2
      ];
...
const safeInstance = {
  ...
  addGoalHistory: async (...args) => {
    try {
      ...
      console.error('SheetsDatabase.addGoalHistory error:', error.message);
      return false;
    }
  },
  ...
  getGoalHistory: async (...args) => {
    try {
      ...
      console.error('SheetsDatabase.getGoalHistory error:', error.message);
      return [];
    }
  },
  ...
  getAllGroups: async (...args) => {
    try {
      ...
      console.error('SheetsDatabase.getAllGroups error:', error.message);
      return [];
    }
  },
  ...
  getCheckChart: async (...args) => {
    try {
      ...
      console.error('SheetsDatabase.getCheckChart error:', error.message);
      return { stages: [] };
    }
  },
  
  saveCheckChart: async (...args) => {
    try {
      if (!instance.initialized) await instance.initialize();
      return await instance.saveCheckChart(...args);
    } catch (error) {
      console.error('SheetsDatabase.saveCheckChart error:', error.message);
      return false;
    }
  },
  
  getStudentCheckProgress: async (...args) => {
    try {
      if (!instance.initialized) await instance.initialize();
      return await instance.getStudentCheckProgress(...args);
    } catch (error) {
      console.error('SheetsDatabase.getStudentCheckProgress error:', error.message);
      return [];
    }
  },
  
  updateStudentCheckProgress: async (...args) => {
    try {
      if (!instance.initialized) await instance.initialize();
      return await instance.updateStudentCheckProgress(...args);
    } catch (error) {
      console.error('SheetsDatabase.updateStudentCheckProgress error:', error.message);
      return false;
    }
  },
  
  getCheckProgressForAllStudents: async (...args) => {
    try {
      if (!instance.initialized) await instance.initialize();
      return await instance.getCheckProgressForAllStudents(...args);
    } catch (error) {
      console.error('SheetsDatabase.getCheckProgressForAllStudents error:', error.message);
      return {};
    }
  },
  ...
  resetAllSessionGoals: async (...args) => {
    try {
      ...
      console.error('SheetsDatabase.resetAllSessionGoals error:', error.message);
      return false;
    }
  },
  
  getSessionData: async (...args) => {
    try {
      if (!instance.initialized) await instance.initialize();
      return await instance.getSessionData(...args);
    } catch (error) {
      console.error('SheetsDatabase.getSessionData error:', error.message);
      return null;
    }
  },
  
  setSessionData: async (...args) => {
    try {
      if (!instance.initialized) await instance.initialize();
      return await instance.setSessionData(...args);
    } catch (error) {
      console.error('SheetsDatabase.setSessionData error:', error.message);
      return false;
    }
  }
};
