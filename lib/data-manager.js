// Data Manager - Hybrid approach with Google Sheets as primary storage
// Falls back to local storage when Sheets is unavailable
const fs = require('fs');
const path = require('path');
const sheetsDB = require('./sheets-database');
...
class DataManager {
  ...
  // Get student profile with Sheets-first, local fallback approach
  async getProfile(studentId) {
    ...
    try {
      ...
      if (dbInitialized) {
        ...
        if (sheetsProfile) {
          console.log(`[DataManager] Profile loaded from Google Sheets:`, sheetsProfile);
          return sheetsProfile;
        }
      }
    } catch (error) {
      console.error(`[DataManager] Error reading from Sheets:`, error);
    }
  ...
  // Save student profile to both Sheets and local storage
  async saveProfile(studentId, profileData) {
    ...
    try {
      const dbInitialized = await sheetsDB.initialize();
      if (dbInitialized) {
        await sheetsDB.updateProfile(studentId, profileData);
        console.log(`[DataManager] Profile saved to Google Sheets`);
      }
    } catch (error) {
      console.error(`[DataManager] Error saving to Sheets:`, error);
    ...
    try {
      ...
      console.log(`[DataManager] Profile saved to local storage as backup`);
